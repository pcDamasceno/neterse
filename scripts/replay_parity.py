...
without the checkpointer plumbing: samples come from the same sources the
...
def main(argv=None) -> int:
    ...
    for command, _platform, body in samples:
        # Parity is defined on the DEFAULT path — platform is a 0.2.0
        # addition the baseline doesn't know; it must never change bytes
        # here, so it is deliberately not passed.
