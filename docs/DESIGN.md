...
(decision 38) — validated live against ACI and SD-WAN fabrics, with
constant-column folding (decision 39) and the `api_shapes` corpus as the
vendor contribution funnel (decision 40). Published on PyPI through
0.5.0; each version's publish awaits the maintainer's tag
(docs/RELEASING.md).
...
| 7 | Consumers swap any vendored copy for the pip dependency; propose a "TOON profile for network data" upstream to toon-format | ◐ published on PyPI through 0.5.0; remaining: toon-format proposal |
| 8 | **Beyond the CLI**: the same candidates contract for other verbose-payload surfaces an agent reads — MCP tool results (including an MCP middleware that compacts any server's responses, the gcf-proxy-shaped adoption vector), REST/API responses, generic JSON documents. The parsed tier is the seam (structured rows are already command-independent); what's new is dispatch keyed on something other than a CLI command string | ◐ MCP surface shipped: `neterse.mcp` — `neterse-mcp` proxy (`--header` for authenticated URL upstreams) + `CompactMiddleware` behind the `mcp` extra (decision 38); vendor envelopes validated live against ACI and SD-WAN fabrics — constant-column folding (decision 39), `tests/fixtures/api_shapes/` as the contribution funnel (decision 40); remaining: per-tool hints for raw-CLI results, REST/API responses, generic JSON documents |
...
| 38 | **MCP compaction lives at the client's config seam: the `neterse-mcp` stdio proxy, plus a FastMCP middleware for servers you author — never in the application** | Phase 8's placement question has three candidate homes, and two fail on ownership grounds. The application layer either already has the verb — an agent loop you own calls `compact()` on the result before it enters context, which phase 5 shipped — or cannot be modified at all (VS Code, Claude Code, Cursor). The server is editable only when you wrote it. The one seam the CONSUMER controls for every server regardless of author is the client's server config, so the shipping artifact is a proxy: `neterse-mcp <url-or-command>` speaks stdio to the host, forwards to the upstream (streamable-HTTP or stdio), and rewrites `tools/call` results on the way back — adoption is an edit to `mcp.json`, the gcf-proxy adoption vector as planned. One pure, stdlib-only seam under both surfaces: `compact_text` claims a text block only when it json-decodes to a CONTAINER (scalars re-encode to themselves; prose that happens to parse must never be touched), routes it through `compact()` (normalizers → parsed tier → smallest-wins), and replaces it only when strictly smaller than the original wire text — often pretty-printed, so the realized saving typically beats the compact-JSON gate (APIC-shaped fixtures: −68…75%). Everything else passes through byte-identical: non-JSON text (the raw tier is keyed on a CLI command string MCP does not carry — per-tool `{tool: (command, platform)}` hints are the recorded next step), JSON scalars, `isError` results, non-text blocks, and input schemas. `structuredContent` was initially left untouched and OPEN; the field answered within the day — FastMCP mirrors every tool return there, VS Code surfaces both copies, and the model read the 9.8 KB original beside the 3.5 KB compacted text, negating the saving — so the resolution is DEDUPE, on by default (`--keep-structured` opts out): a `{"result": <text>}` string mirror is rewritten with the compacted text (any string satisfies its schema), a copy that deep-equals what the compacted block decoded to is dropped, and anything carrying information of its own is never touched. Dropping obliged a matching listing change: the MCP server layer inside the proxy rejects any result whose tool declares `outputSchema` but returns no structured output (the low-level SDK validates against its `tools/list` cache, which is fed through the middleware chain), so while dedupe is on, listed tools shed `output_schema` — the proxy's honest contract is "the text block is the payload". That interaction was invisible to the in-memory suite (a bare `-> dict` tool gets no inferred schema in fastmcp 3) and surfaced only through the live proxy; the fixture now declares an explicit `output_schema`, the same lesson as decision 35's vendor-decoder differential: the enforcement path, not our reading of it, is the test. fastmcp (`>=3`) is the `mcp` extra and powers only the adapters; `import neterse.mcp` works without it and `CompactMiddleware` names the missing extra at instantiation instead. (An environment marker briefly gated the extra to Python 3.10+ for the sake of 3.9 resolution; 0.5.0 raised `requires-python` to 3.11 — 3.9 is EOL October 2026 and the marker plus the packaging tests' `tomli` fallback were the only things the old floor bought — so the extra is unconditional.) The stdio upstream inherits the proxy's FULL environment: the host applied the config's `env` block to the proxy process, and the MCP SDK's default child env is a scrubbed allowlist that would starve a `docker run -e` upstream. Dev-mode invocations (`uv run --with-editable …`) additionally need `--isolated`: spawned from a workspace whose base venv holds ANY installed `neterse`, uv's environment layering can resolve a CHIMERA — `neterse.mcp` imported from the repo while `neterse.parsed`/`compact` come from the stale venv install (observed live, 2026-08-06: the proxy ran 0.4.1's parsed tier under 0.5.0's middleware and silently shed the current improvements — the user-visible symptom was "the ACI results aren't using the improvement") — and `--isolated` pins every import to the editable tree. The published-package invocation (`uvx --from 'neterse[mcp]' neterse-mcp …`) is inherently isolated and does not need the flag. An authenticated URL upstream moves its client-config `headers` block onto the proxy CLI as repeatable curl-style `--header 'Name: value'` flags; a `--header` on a stdio upstream is refused loudly rather than silently dropped, because an unauthenticated upstream with no hint why is the worse failure. Weighed and dropped: teaching `compact()` itself to sniff JSON strings (parity-safe — corpus raw text never json-decodes — but widening the one verb's contract is its own decision); compacting `resources/read` (same seam, later). |
| 39 | **Constant-column folding: a leading `[N rows, each: col=value,...]` line — new `parsed:fold` candidate at top level, the same fold inside `parsed:sections` tables** | Live validation against a real ACI fabric and a real SD-WAN overlay (2026-08-06, through the proxy) showed the header-once tables still repeating what controllers repeat: every object of an APIC class response carries the same class name and a dozen scheme defaults (`fvTenant` ×20, `lcOwn=local` ×20, runs of empty cells), SD-WAN device lists repeat site-wide versions. A column whose RENDERED cell is identical in every row states one value N times; the fold states it once in a leading marker — the omission marker's bracket idiom but the opposite promise: nothing omitted, and the row count included, because N identical rows must stay distinguishable from one when EVERY column folds and no row line remains. Lossless by construction, declared so (`dropped_fields=()`), values quoted like the CSV cells they replace. Placement is byte-discipline: at top level it is a NEW candidate (`parsed:csv` unchanged — candidates, not policy; smallest-wins takes the fold only when it wins), while inside sections' tables (the MCP envelope path, where the redundancy actually arrives) it replaces the rendering only under an exact both-forms size comparison — the per-column test (`N×(cell+1) > pair+1`) is a preselector, the strict whole-table comparison is the final word — so a short constant on few rows correctly declines against the marker's fixed overhead and EVERY pre-existing pinned fixture renders byte-identically: the whole suite passed unmodified, zero re-pins, token baseline untouched. In `_table_lines`, a fold must leave varying columns behind (each row line anchors its indented sub-tables); the all-constant degenerate form is flat-table business. `parsed:toon`/`parsed:gcf` never fold — neither spec has the construct, and conformance is their point. Numbers over the 16-tool live corpus: −52.9% → −61.0% chars vs raw wire text, −46.2% real o200k_base tokens; ACI bridge domains 30,190→6,617 chars (−67.3% → −78.1%), tenants −56.2% → −70.0%; SD-WAN devices −54.7% → −63.5%; identity fidelity re-verified through the proxy (every name/dn/system-ip present verbatim). Pinned in `tests/test_parsed_only.py`; the live payloads live on in `tests/fixtures/api_shapes/` (decision 40). |
| 40 | **Vendor API shapes stay code — tiny normalizers behind a fixture corpus; the YAML-subtree DSL is declined** | The contribution question: as agent users meet other controllers (Meraki, DNAC, NetBox, Mist, PAN-OS), should vendor envelopes be declared in YAML like `specs/` — a sniff plus a "subtree" pointer to the rows — so contributing never means Python? Validated against two live fabrics and answered NO, on evidence. The generic engine already finds rows wherever they nest: `_section_lines` recursion plus the `normalize()` seam reached ACI's `imdata` THROUGH the tool wrapper `{endpoint, method, data:{totalCount, imdata}}` with zero new code, and the SD-WAN server's `{count, returned, by_*:{...}, data:[...]}` envelope compacted −31…63% with zero vendor code anywhere in the tree — a YAML saying "rows live at `data`" would document what recursion already discovers. What IS vendor-specific is per-item transformation (ACI's `{class: {attributes: {...}}}` unwrap synthesizing a collision-safe `_class` column), and a transform is where a declarative format stops being one: CONTRIBUTING's spec-key rule — "a key that amounts to 'run this little program' is a code compressor" — applies with more force here, because envelope styles mutate per API version and the DSL would grow a primitive per mutation. The contribution surface instead, cheapest first: (1) drop a real capture in `tests/fixtures/api_shapes/<vendor>/<name>.json` — `tests/test_api_shapes.py` auto-asserts it shrinks below compact JSON, the winner declares lossless, every identity value (name/dn/address) survives verbatim, and nothing raises, with no per-vendor test code; (2) only when the generic result disappoints, a ~30-line normalizer module plus one `NORMALIZERS` line (`cisco_aci.py` is the whole template); (3) out-of-tree, `register_normalizer` wins without a PR. YAML keeps its franchise where content is CONFIG rather than transform: `specs/` for CLI text, and phase 8's recorded per-tool hints file (`{tool: (command, platform)}`) when raw-CLI-in-JSON results arrive. |
