"""FROZEN Phase-0 parity baseline — DO NOT EDIT THE CODE.

Verbatim copy of the original in-agent ``token_optimizer`` module as it
stood before the extraction into the terse package (the only edit is the
...
# The ONLY functional edit vs the original: stdlib logger instead of the
# host application's get_logger, so the snapshot imports standalone.
